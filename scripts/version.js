...
async function prepare(pluginConfig, { nextRelease: { version }, logger }) {
	const path = pluginConfig && pluginConfig.path ? pluginConfig.path : 'src/version.ts';
	const number = getVersionNumber(version);
	version = version.replace('-', '.');

	await updateVersionFile(version, number, logger, path);
	await updateFiles(version, number);
	await updateReactNative(version, number);
}

async function updateVersionFile(version, versionCode, logger, path) {
